...
**SUPERSEDED (pending Karen clarifications) — 2026-07-21:** Karen sent a simpler **2-question "Apex" decision tree** for workout slotting: **Q1** home vs commercial gym → Apex Home / Apex Gym; **Q2** 30 vs 60 min → Apex30 (pre-recorded) or, for 60, a choice of Live group strength (Zoom) or Apex60 (pre-recorded). Programs are `Apex30 A/B…` and `Apex60 A/B…`, each 12 weeks; the A/B/C **letter is the continuation arc**, not a join-time axis. This **drops skill-level and collapses the equipment axis**, so it partly contradicts the 4-axis key above. Commercial-gym content does not exist until Q3/Q4 2026. Keep the "same key → same program, deterministic, no model selection" rule; the axes are what changed. Full tree, reconciliation, and open questions in the spec doc ("Program-slotting decision tree — Karen 2026-07-21") and memory `[[apex_program_slotting_tree]]`. Do not build the 4-axis picker as spec'd above without reconciling against the Apex tree first.
