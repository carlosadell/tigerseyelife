...
export default function VerifyMembershipScreen() {
  ...
  const callEdge = async <T,>(body: object): Promise<T> => {
    ...
    const { data, error: invokeError } = await supabase.functions.invoke<T>(
      "verify-membership",
      { body },
    );
  ...
  const onSendCode = async () => {
    ...
    try {
      ...
      const result = await callEdge<StartResponse>({
        mode: "start",
        email: trimmed,
      });
      if (!result.found) {
      ...
      console.warn("Could not start membership verification", e);
      setError(
        "We couldn't send a code right now. Please try again in a minute or contact support.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const onConfirm = async () => {
    ...
    try {
      if (isDevSession || !supabase) {
        ...
        router.replace("/onboarding/intake/age");
        return;
      }
      const result = await callEdge<ConfirmResponse>({
        mode: "confirm",
        email: trimmed,
        code: code.trim(),
      });
      ...
      console.warn("Could not confirm membership verification", e);
      setError(
        "We couldn't verify that code right now. Please try again or contact support.",
      );
    } finally {
      setSubmitting(false);
    }
        ...
        <View style={styles.frame}>
          ...
          >
            <PhotoHeroCard
              ...
              title={
                stage === "email"
                  ? "Which email did you use to join?"
                  : stage === "code"
                    ? "Check your inbox."
                    : stage === "not_found"
            ...
            {stage === "email" && (
              <>
                <Text style={styles.body}>
                  We'll send a six-digit code to confirm. Your app login can be
                  a different email. We just need the one you used for Create
                  Power.
                </Text>
                ...
                {isDevSession && (
                  <Text style={styles.hint}>
                    Dev mode: any email with "+member" matches. OTP is 123456.
                  </Text>
                )}
              </>
            )}
            ...
            {stage === "code" && (
              <>
                <Text style={styles.body}>
                  We sent a code to {email.trim().toLowerCase()}. It expires in
                  10 minutes.
                </Text>
                ...
                <Pressable onPress={() => setStage("email")} hitSlop={8}>
                  <Text style={[styles.hint, { color: COLORS.tigerGold }]}>
            ...
            {stage === "not_found" && (
              <Text style={styles.body}>
                Double-check the email you used to join Create Power. If it
                still doesn't work, you can continue as a non-member or contact
                Tigers Eye Life for help.
              </Text>
            )}
            ...
            {stage === "email" && (
              ...
              >
                <Text
                  style={[
                    styles.ctaText,
                    { color: ctaTextOnTangerine("light") },
                  ]}
                >
